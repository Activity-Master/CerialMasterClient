...
public class MultiTimedComPortSender {
    ...
    private final java.util.Set<MultiEmitter<? super ManagerStatus>> statusEmitters = java.util.Collections.newSetFromMap(
            new java.util.concurrent.ConcurrentHashMap<>());

    private final Multi<ManagerMessageProgress> progress;
    private final Multi<ManagerMessageProgress> rawProgress;
    private final java.util.Set<MultiEmitter<? super ManagerMessageProgress>> progressEmitters = java.util.Collections.newSetFromMap(
            new java.util.concurrent.ConcurrentHashMap<>());
    ...
    private final java.util.concurrent.ScheduledExecutorService eventScheduler = java.util.concurrent.Executors.newSingleThreadScheduledExecutor(
            r -> {
    ...
    private final java.util.concurrent.atomic.AtomicBoolean aggregateFinalized = new java.util.concurrent.atomic.AtomicBoolean(
            false);
    ...
    public java.lang.AutoCloseable onStatisticsUpdated(java.util.function.Consumer<com.guicedee.activitymaster.cerialmaster.client.ManagerSnapshot> consumer) {
        ...
        try {
            ...
            final com.guicedee.activitymaster.cerialmaster.client.ManagerSnapshot snap = buildManagerSnapshot(agg,
                                                                                                              25,
                                                                                                              100
            );
            Infrastructure
                    ...
                    .execute(() -> {
                        try {
                            snap
                                    .getPerSender()
                                    .forEach((port, snapshot) -> {
                                        snapshot
                                                .getWaiting()
                                                .clear();
                                        snapshot
                                                .getCompleted()
                                                .removeIf(a -> !a.isErrored());
                                    });
    ...
    private void resetGroupCompletionHandlers() {
        try {
            ...
            try {
                groupCompletionSubscribers.clear();
            } catch (Throwable ignored) {
            }
        }
    }
                ...
                .streams(
                        Multi
                                ...
                                .item(new ManagerMessageProgress(null,
                                                                 new MessageProgress(null,
                                                                                     null,
                                                                                     null,
                                                                                     0,
                                                                                     TimedComPortSender.State.Idle,
                                                                                     new Config(),
                                                                                     new Config(),
                                                                                     "Subscribed"
                                                                 )
    ...
    public TimedComPortSender getOrCreateSender(Integer comPort, Config baseConfig) {
        ...
        try {
            com.guicedee.activitymaster.cerialmaster.client.services.ICerialMasterService<?> svc = com.guicedee.client.IGuiceContext.get(
                    com.guicedee.activitymaster.cerialmaster.client.services.ICerialMasterService.class);
            log.trace("📚 Retrieving ComPortConnection via service for COM{}", comPort);
            // Wait up to 50s as other tests do; environment dependent
            conn = svc
                    .getComPortConnectionDirect(comPort)
                    .await()
        ...
        try {
            if (this.beforeStartConfig != null) {
                s.setBeforeStartConfig(this.beforeStartConfig);
            }
        } catch (Throwable ignored) {
        }
    ...
    private void cancelAllAndReset(String reason, long settleMillis) {
        ...
        try {
            synchronized (aggLock) {
                ...
                try {
                    if (aggFuture != null && !aggFuture.isDone()) {
                        aggFuture.complete(null);
                    }
                } catch (Throwable ignored) {
                }
                aggFuture = null;
                try {
                    aggregateFinalized.set(false);
                } catch (Throwable ignored) {
                }
            }
    ...
    private Uni<GroupResult> enqueueGroup(Integer comPort, List<MessageSpec> specs, Config baseConfig, boolean standalone) {
        ...
        synchronized (aggLock) {
            ...
            log.trace("Checking mustReset for COM{}: standalone={}, runActive={}, plannedByPortKeys={}, mustReset={}",
                      comPort,
                      standalone,
                      runActive,
                      (plannedByPort != null ? plannedByPort.keySet() : "null"),
                      mustReset
            );
        }
        if (mustReset) {
            log.debug("Preempting existing run before enqueueing single-port group on COM{} (standalone conflict)",
                      comPort
            );
        ...
        log.trace("🚀 Enqueueing group on COM{} with {} messages{}",
                  comPort,
                  count,
                  pausedAll.get() ? " (manager paused)" : ""
        );
        ...
        if (shouldInitSinglePortPlan) {
            try {
                ...
                try {
                    sender.resetForRetry("New group starting – status reset");
                } catch (Throwable ignored) {
                }
                publishSenderSummary(comPort);
            } catch (Throwable ignored) {
        ...
        if (sender.hasActiveGroupOrMessage()) {
            try {
                sender.cancel("New group enqueued – cancelling current", true);
            } catch (Throwable ignored) {
            }
            return Uni
                    .createFrom()
                    ...
                    .onItem()
                    .transformToUni(v -> sender.enqueueGroup(specs));
        }
    ...
    public Uni<Map<Integer, GroupResult>> enqueueGroups(Map<Integer, List<MessageSpec>> byPort,
        ...
        if (rateHit.isPresent()) {
            String reason = rateHit.get();
            log.warn("⛔ {} – stopping processing and rejecting multi-port enqueue ({} ports)",
                     reason,
                     byPort == null ? 0 : byPort.size()
            );
            try {
                cancelAllAndReset(reason, 100L);
            } catch (Throwable ignored) {
        ...
        if (preemptExisting) {
            try {
                cancelAllAndReset("New multi-port group enqueued – resetting state", 100L);
            } catch (Throwable ignored) {
            }
        }
        if (byPort == null || byPort.isEmpty()) {
            ...
            synchronized (aggLock) {
                ...
                this.plannedByPort = new LinkedHashMap<>();
                this.plannedBaseConfig = baseConfig != null ? baseConfig : new Config();
                this.plannedGroupProperties = (properties == null) ? java.util.Map.of() : new java.util.LinkedHashMap<>(
                        properties);
                this.attemptsByMessage.clear();
                this.terminalMessages.clear();
                this.messageStartNanos.clear();
        ...
        Uni<Map<Integer, GroupResult>> combined = Uni
                .combine()
                .all()
                .unis(unis)
                .with(results -> {
    ...
    private void onChildMessageProgress(Integer port, MessageProgress mp) {
        ...
        if (mp.state == TimedComPortSender.State.Completed || mp.state == TimedComPortSender.State.TimedOut
            ...
            if (newlyTerminal) {
                ...
                if (mp.state != TimedComPortSender.State.Completed) {
                    ...
                    try {
                        ...
                        if (s != null) {
                            try {
                                statusAtFailure = s.getCurrentStatus();
            ...
            // If this message just reached terminal, check if all planned messages are now terminal.
            // Only auto-finalize here for single-port runs; for multi-port runs we finalize via the
            // combined Uni in enqueueGroups() once ALL ports have finished. This ensures
            // onGroupCompleted fires only once at the end of the entire group, not per port.
            try {
                Map<Integer, List<MessageSpec>> byPort = this.plannedByPort;
                if (byPort != null && !byPort.isEmpty()) {
                    int plannedTotal = byPort
                            .values()
                            .stream()
    ...
    private void finalizeAggregate() {
        ...
        try {
            ...
            Set<String> failedIds = this.failures
                    .stream()
                    .map(fl -> fl.messageId)
        ...
        try {
            ...
            emitProgress(null,
                         new MessageProgress(null,
                                             ...
                                             null,
                                             0,
                                             TimedComPortSender.State.Completed,
                                             cfg,
                                             cfg,
                                             "Run finalized"
                         )
    ...
    public AggregateProgress computeAggregateSnapshot(boolean finishing) {
        ...
        int totalMessages = byPort
                .values()
                .stream()
                .mapToInt(l -> l == null ? 0 : l.size())
                .sum();
        long totalTasksBudget = byPort
                .entrySet()
                .stream()
                .mapToLong(e -> e
                        .getValue()
        ...
        for (Map.Entry<Integer, List<MessageSpec>> entry : byPort.entrySet()) {
            ...
            for (MessageSpec spec : specs) {
                ...
                long worstForMsgMs = isTerminal ? 0 : (remainingRetries * delayMsForMessage(spec) + timeoutMsForMessage(
                        spec));
            ...
            boolean portAnyErrored = failures
                    .stream()
                    .anyMatch(f -> Objects.equals(f.comPort, port));
            boolean portAnyWaitingForResponse = messageIdToPort
                    .values()
                    .stream()
            ...
            perPort.put(port,
                        new PortDetail(port,
                                       ...
                                       portBudget,
                                       portRemainingTasks,
                                       portWorstRemainingMs,
                                       portWorstRemainingMs,
                                       portAnyErrored,
                                       portAnyWaitingForResponse
                        )
            );
        ...
        Map<String, Boolean> props = plannedGroupProperties != null ? java.util.Collections.unmodifiableMap(new java.util.LinkedHashMap<>(
                plannedGroupProperties)) : java.util.Map.of();
    ...
    public java.lang.AutoCloseable onGroupCompleted(java.util.function.Consumer<AggregateProgress> consumer,
        ...
        return () -> {
            try {
                ...
                if (c != null) {
                    c.cancel();
                    groupCompletionSubscribers.remove(c);
                }
            } catch (Throwable ignored) {
            }
        };
    ...
    public Uni<Void> cancelAll(String reason) {
        ...
        emitStatus(null,
                   TimedComPortSender.State.Cancelled,
                   "Manager cancelAll: " + (reason == null ? "Cancelled" : reason)
        );
        // Propagate cancellation to all senders
        senders
                .values()
                .forEach(s -> s.cancel(reason));
    ...
    public com.guicedee.activitymaster.cerialmaster.client.ManagerSnapshot snapshot(int waitingLimit, int completedLimit) {
        ...
        return new com.guicedee.activitymaster.cerialmaster.client.ManagerSnapshot(agg,
    ...
    private void scheduleAggregatePublish() {
        ...
        final com.guicedee.activitymaster.cerialmaster.client.ManagerSnapshot managerSnap = buildManagerSnapshot(
                aggSnapshot,
                25,
                100
        );
    ...
    private String fingerprintSenderSnapshot(SenderSnapshot s) {
        ...
        String sendingId = (s.getSending() == null || s
                .getSending()
                .getId() == null) ? "-" : s
                .getSending()
                .getId();
        String sendingTitle = (s.getSending() == null || s
                .getSending()
                .getTitle() == null) ? "-" : s
                .getSending()
                .getTitle();
    ...
    private String fingerprintManagerSnapshot(com.guicedee.activitymaster.cerialmaster.client.ManagerSnapshot snap) {
        ...
        sb
                .append(fingerprintAggregate(snap.aggregate))
                .append('|');
    ...
    public java.util.Optional<MessageSpec> markCompletedReturningSpec(String messageId) {
        ...
        if (port != null) {
            ...
            if (s != null) {
                ...
                if (snap != null && snap.getSending() != null && messageId.equals(snap
                                                                                          .getSending()
                                                                                          .getId())) {
    ...
    public java.util.Optional<MessageSpec> markCompletedReturningSpec(int comPort) {
        ...
        if (snap != null && snap.sending != null) {
            ...
            MessageSpec reconstructed = new MessageSpec(ms.getId(),
                                                        ms.getTitle(),
                                                        ms.getPayload(),
                                                        ms.getEffectiveConfig()
            );
